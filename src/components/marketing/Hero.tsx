...
export function Hero() {
  return (
    <section className="relative overflow-hidden">
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_top,var(--accent-bg),transparent_60%)]" aria-hidden="true" />
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-10 px-4 py-20 text-center sm:px-6 lg:py-28">
        ...
        >
          <QrCode className="h-4 w-4" /> Now in beta for Ganesh mandals across Maharashtra
        </motion.span>
        ...
        >
          Every donation deserves a <span className="text-[var(--accent)]">beautiful receipt.</span>
        </motion.h1>
        ...
        >
          Utsavam replaces the register-and-receipt-book with a digital flow: your collectors record a
          donation, the donor gets a shareable QR receipt in seconds — no app, no account, no friction.
        </motion.p>
        ...
        >
          <Link to={ROUTES.demo}>
            <Button size="lg">
              <PlayCircle className="h-5 w-5" /> Try for Free — no signup
            </Button>
          </Link>
